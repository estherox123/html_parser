...
def get_existing_html_files(repo_url, folder_name):
    # GitHub API endpoint for repository contents
    contents_url = urljoin(repo_url, 'tree/main/' + folder_name + '_HTML')
    ...
    # Filter out only the HTML files and return their names
    html_files = [file['name'] for file in files_data if file['name'].endswith('.html')]
    return html_files

def update_navigation_page(output_folder, folder_name):
    index_file_path = os.path.join(output_folder, 'index.html')
    
    # Assume your GitHub repository URL is something like the following:
    repo_url = 'https://github.com/estherox123/html_parser'
    existing_html_files = get_existing_html_files(repo_url, folder_name)
    ...
    # Include only existing HTML files
    for html_file in existing_html_files:
        url_encoded_file_name = urllib.parse.quote(html_file)
        relative_path = f"{folder_name}/{url_encoded_file_name}"
        index_content += f"        <li><a href='{relative_path}'>{html_file}</a></li>\n"
...
def code():
    ...
    update_navigation_page('https://github.com/estherox123/html_parser', keyword)
